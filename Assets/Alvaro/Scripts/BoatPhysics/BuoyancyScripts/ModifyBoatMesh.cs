...
{
    //The boat transform needed to get the global position a vertex
    private Transform boatTrans;
    //Coordinates of all in the original boat
    Vector3[] boatVertices;
    //Positions in allVerticesArray to build triangles
    int[] boatTriangles;
    //The boat's rigidbody
    private Rigidbody boatRB;

    //So we only need to make the transformtion from local to global once
    public Vector3[] boatVerticesGlobal;
    //Find all the distance to water once because some triangles share vertices, so reuse
    ...
    public void GenerateUnderwaterMesh()
    {
        ...
        for(int j = 0; j < boatVertices.Length; j++)
        {
            ...
            //And if we want debug, we can convert it back to local
    ...
    private void AddTriangles()
    {
        ...
        while(i < boatTriangles.Length)
        {
            ...
            else //1 or 2 vertices are below the water
            {
                //Sort the vertices

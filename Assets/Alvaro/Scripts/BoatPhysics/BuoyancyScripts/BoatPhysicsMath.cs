...
public static class BoatPhysicsMath
{
    ...
    public static float ResistanceCoefficient(float rho, float velocity, float length)
    {
        ...
        //Viscosity depends of the temperature but at 20 degrees celcius:
        float nu = 0.000001f;
        //At 30 degrees celcius: nu = 0.000008f; so no big difference
    ...
    {
        //To capture the response of the fluid to sudden acceleration or penetrations

        //Add slamming if the normal is in the same direction as the velocity (the triangle is not receding from the water)
        //Also make sure the area is not 0, which it sometimes is for some reason
    ...
    //
    // Resistance forces from the book "Physics for Game Devolopers"
    //

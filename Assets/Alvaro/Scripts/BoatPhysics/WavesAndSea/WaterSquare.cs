...
public class WaterSquare
{
    ...
    public void MoveSea(Vector3 oceanPos, float timeSinceStart)
    {
        ...
        for(int i = 0; i < vertices.Length; i++)
        {
            ...
            //Vector3 vertexGlobalTest = squareTransform.rotation * Vector3.Scale(vertex, squareTransform.localScale) + squareTransform.position;

            //Debug
            if(i == 0)
            {
                //print(vertexGlobal + " " + vertexGlobalTest);
            }

            //Get the water height at this coordinate
            vertex.y = WaterController.current.GetWaveYPos(vertexGlobal, timeSinceStart);

            //From global to local - not needed if we use the save local x, z position
    ...
    public void GenerateMesh()
    {
        ...
        for(int z = 0; z < width; z++)
        {
            ...
            for(int x = 0; x < width; x++)
            {
                ...
                //The triangle south-west of the vertex
                tris.Add(x + z * width);
                tris.Add(x + (z-1) * width);
                tris.Add((x-1) + (z-1) * width);

                //The triangle west-south of the vertex
                tris.Add(x + z * width);
                tris.Add((x-1) + (z-1) * width);
